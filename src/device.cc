...
t_device::t_device(t_session* a_session, ALCdevice* a_device, ALCcontext* a_default) : t_base_device(a_session->v_devices.emplace(a_device, t_object::f_of(this)).first), v_default(a_default)
{
	...
	f_new<t_context>(a_session->v_library, this, v_default);
}

t_pvalue t_device::f_create_buffer(ALuint a_id)
{
	return f_new<t_buffer>(t_session::f_instance()->v_library, this, a_id);
}
...
t_pvalue t_device::f_create_context()
{
	...
	return f_new<t_context>(session->v_library, this, context);
}
...
void t_type_of<xemmaix::al::t_base_device>::f_define(t_library* a_library)
{
...
void t_type_of<xemmaix::al::t_device>::f_define(t_library* a_library)
{
	using namespace xemmaix::al;
	t_define{a_library}
...
void t_type_of<xemmaix::al::t_capture_device>::f_define(t_library* a_library)
{
	using namespace xemmaix::al;
	t_define{a_library}
